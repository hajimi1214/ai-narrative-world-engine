...
import { api, apiUrl } from "../../app/lib";
...
type LiveSession = { id: string; label: string; model?: string | null; phase: string; message: string; preview: string; total_chars: number; status: string; started_at: string; updated_at: string; error_code?: string | null };
...
export function AiLivePanel({ projectId }: { projectId: string }) {
  const [open, setOpen] = useState(false); const [traces, setTraces] = useState<Trace[]>([]); const [liveSessions, setLiveSessions] = useState<LiveSession[]>([]); const [collapsed, setCollapsed] = useState(false);
  const load = async () => {
    try {
      const [execution, autoRuns, live] = await Promise.all([
        ...
        api(`/projects/${projectId}/ai-live`).catch(() => []) as Promise<LiveSession[]>,
      ]);
  ...
  useEffect(() => {
    const stream = new EventSource(`${apiUrl}/projects/${projectId}/ai-live/stream`);
    const receiveSnapshot = (event: Event) => {
      try { setLiveSessions((JSON.parse((event as MessageEvent).data).sessions || []) as LiveSession[]); } catch { /* Polling remains the recovery path. */ }
    };
    stream.addEventListener("snapshot", receiveSnapshot);
    return () => { stream.removeEventListener("snapshot", receiveSnapshot); stream.close(); };
  }, [projectId]);
  const active = useMemo(() => [...traces.filter((trace) => trace.status === "STARTED"), ...liveSessions.filter((session) => session.status === "RUNNING")], [traces, liveSessions]);
  return <>
