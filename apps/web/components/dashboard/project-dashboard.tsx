...
function WorkflowOverview({ projectId, data }: { projectId: string; data: any }) {
  ...
  const nextLabel = next?.task?.number ? `继续第 ${next.task.number} 章：${next.task.title || "打开任务单"}` : "启动自动导演：自动生成整书规划与正文";
  const nextHref = `/projects/${projectId}/${next?.task?.number ? "chapters" : "director/auto"}`;
...
function EmptyWorld({ projectId, data }: { projectId:string; data:any }) { const setup = [{label:"整本规划", value:0, href:"planning", action:"手动编辑规划"}, {label:"人物", value:(data.active_characters || []).length, href:"characters", action:"建立行动者"}, {label:"世界真相", value:(data.canon || []).length, href:"canon", action:"设定 Canon"}, {label:"剧情压力", value:(data.active_story_threads || []).length, href:"story-threads", action:"创建线程"}]; return <><section className="hub-blank"><div className="hub-blank-mark"><Sparkles size={28} /></div><div><p className="eyebrow">START WITH AI</p><h2>让自动导演生成整本书和正文。</h2><p>设置整本章节数与本次生成数量后，AI 会依次生成方向、人物、世界、卷纲、章节任务、正文、审核与修复。</p></div></section><section className="world-setup-path">{setup.map((item, index) => <Link href={`/projects/${projectId}/${item.href}`} key={item.label}><span className="path-number">0{index + 1}</span><div><small>{item.label}</small><strong>{item.value ? "已准备" : item.action}</strong></div><ArrowRight size={16} /></Link>)}<Link className="path-run" href={`/projects/${projectId}/director/auto`}><WandSparkles size={18} /><span>启动自动导演</span></Link></section><StatGrid projectId={projectId} data={data} /></>; }
