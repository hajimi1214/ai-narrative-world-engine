...
export function ProjectShell({ projectId, children }: { projectId: string; children: ReactNode }) {
  ...
  const savePageDraft = () => {
    ...
    if (values.length) localStorage.setItem(draftKey(), JSON.stringify({ version: 1, saved_at: new Date().toISOString(), values }));
    setSavedAt(Date.now());
  };
  useEffect(() => {
