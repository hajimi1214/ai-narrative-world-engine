...
export function ProjectShell({ projectId, children }: { projectId: string; children: ReactNode }) {
  ...
  const nav = <nav className="sidebar-nav">{groups.map((group) => <div className="nav-group" key={group.label}><span className="nav-label">{group.label}</span>{group.items.map(([name, route, Icon, disabled]) => { const href = `/projects/${projectId}${route ? `/${route}` : ""}`; const active = route === "" ? pathname === href : current === route; return disabled ? <span className="nav-item disabled" title="规划中" key={route}><Icon size={18} /><b>{name}</b></span> : <Link className={`nav-item ${active ? "active" : ""}`} href={href} key={route} onClick={() => setDrawer(false)}><Icon size={18} /><b>{name}</b></Link>; })}</div>)}</nav>;
  return <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}><aside className={`sidebar ${drawer ? "drawer-open" : ""}`}><div className="brand-lockup"><BrainCircuit size={24} /><div><strong>AI 自演小说世界引擎</strong><small>Narrative World Engine</small></div></div>{nav}<button className="collapse-button" onClick={toggle} aria-label="收起侧边栏">{collapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}</button></aside>{drawer && <button className="drawer-backdrop" aria-label="关闭菜单" onClick={() => setDrawer(false)} /> }<div className="work-area"><header className="workspace-topbar"><button className="mobile-menu" onClick={() => setDrawer(true)}><Menu size={20} /></button><div className="breadcrumb"><Link href="/">项目</Link><span>/</span><strong>{project?.name || "加载项目…"}</strong></div><div className="world-status"><span><CircleDot size={14} /> {project?.status === "ACTIVE" ? "世界运行中" : "世界已暂停"}</span><span>世界时间：{project?.current_world_time || "尚未建立"}</span></div></header><main className="main-content">{children}</main></div></div>;
}
