...
from app.causal_ledger import (
    ...
    SceneStateTransitionExtractor, read_overlay_path,
)
from app.db import Base
from app.main import app
from app.models import (
    CausalEdgeKind, CausalLink, CausalRelationType, CharacterKnowledge,
    RetconApplication, RetconReplaySession, Scene, SceneCommit, SceneStateCheckpoint,
    TimelineEvent, TimelineEventType, WorldSnapshot,
)
...
def test_explicit_knowledge_informs_decision_only(session, monkeypatch):
    ...
    decision = session.get(__import__("app.models", fromlist=["CharacterDecision"]).CharacterDecision, turn.character_decision_id)
    decision.knowledge_used = [knowledge.id]
    session.commit()
...
def test_ledger_failure_rolls_back_normal_materialization(session, monkeypatch):
    project, _location, _actor, _other, _proposal, performance, _turn, _resolution, _batch, client = _commit(session, monkeypatch)
    counts = {model: session.query(model).count() for model in (Scene, SceneCommit, TimelineEvent, CausalLink)}
    def fail(stage):
    ...
    assert counts == {model: session.query(model).count() for model in counts}
...
def test_replay_ledger_failure_rolls_back_derived_rows(session, monkeypatch):
    ...
    session.expire_all()
    assert counts == (session.query(TimelineEvent).count(), session.query(CausalLink).count(), session.query(Scene).count(), session.query(WorldSnapshot).count())
