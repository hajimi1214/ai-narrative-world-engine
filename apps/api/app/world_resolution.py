...
class HeuristicWorldResolver:
    def resolve(self, context: dict[str, Any]) -> tuple[dict[str, Any], ModelResult | None]:
        ...
        if request.get("kind") == "MOVE":
            return {"outcome":"SUCCESS", "outcome_summary":"The character moves within the established location.", "objective_facts":[{"subject_type":"LOCATION", "subject_id":entity["id"], "predicate":"movement.observed", "value":True}], "state_effects":[], "actor_observation":"The character follows an available path through the location.", "public_observation":"Movement within the established location is observed.", "canon_fact_ids_used":[], "world_entity_ids_used":[entity["id"]], "resolution_basis_summary":"The location is an established world entity; this records movement only.", "missing_information":[]}, None
