        ...
        for run in final_runs.values():
            ...
            if decisions:
                ...
                for idx, item in enumerate(decisions, 1):
                    ...
                    if not staged_turn:
                        self._fail("REPLAY_EXECUTION_LINEAGE_INCOMPLETE")
                    if list(staged_turn.get("scene_beat_refs", [])) != list(action.get("scene_beat_refs", [])):
                        self._fail("REPLAY_SCENE_BEAT_LINEAGE_INVALID")
                    turn = ScenePerformanceTurn(project_id=session.project_id, performance_id=performance.id, sequence=staged_turn["sequence"], actor_character_id=item["character_id"], actor_context_fingerprint=item["context_fingerprint"], character_decision_id=decision.id, action_visibility=staged_turn.get("visibility", action["visibility"]), observable_action=staged_turn.get("observable_action", action.get("observable_action")), spoken_content=staged_turn.get("spoken_content", action.get("spoken_content")), recipient_character_ids=staged_turn.get("recipient_character_ids", []), requires_world_resolution=staged_turn.get("requires_world_resolution", action.get("requires_world_resolution", False)), world_resolution_request=staged_turn.get("world_resolution_request", action.get("world_resolution_request")), scene_beat_refs=staged_turn.get("scene_beat_refs", action.get("scene_beat_refs", [])), validation_result=staged_turn.get("validation", {"valid": True}), replay_session_id=session.id, replay_of_id=staged_turn.get("replay_of_id"))
