    ...
    def build(self, session: Session, project_id: str, character_id: str, proposal: SceneProposal, performance_id: str, turns: list[Any]) -> dict[str, Any]:
        ...
        for turn in turns:
            ...
            if turn.actor_character_id == character_id:
                ...
                own.append({"turn": turn.sequence, "decision_type": getattr(decision.decision_type, "value", decision.decision_type) if decision else None, "chosen_action": decision.chosen_action if decision else None, "target_character_id": decision.target_character_id if decision else None, "observable_action": turn.observable_action, "spoken_content": turn.spoken_content})
