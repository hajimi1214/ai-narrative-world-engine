"""world version and execution foundation"""
...
def upgrade():
    ...
    op.create_table("execution_traces",sa.Column("id",sa.String(36),primary_key=True),sa.Column("project_id",sa.String(36),sa.ForeignKey("projects.id"),nullable=False),sa.Column("stage",sa.String(50),nullable=False),sa.Column("source_type",sa.String(100)),sa.Column("source_id",sa.String(36)),sa.Column("status",sa.String(30),nullable=False),sa.Column("provider",sa.String(100)),sa.Column("model",sa.String(200)),sa.Column("input_fingerprint",sa.String(100)),sa.Column("output_fingerprint",sa.String(100)),sa.Column("latency_ms",sa.Integer()),sa.Column("request_id",sa.String(200)),sa.Column("error_type",sa.String(100)),sa.Column("error_code",sa.String(100)),sa.Column("upstream_status",sa.Integer()),sa.Column("validation_report",sa.JSON(),nullable=False),sa.Column("repairable",sa.Boolean(),nullable=False),sa.Column("retryable",sa.Boolean(),nullable=False),sa.Column("attempt_number",sa.Integer(),nullable=False),sa.Column("parent_trace_id",sa.String(36),sa.ForeignKey("execution_traces.id")),sa.Column("created_at",sa.DateTime(),server_default=sa.text("now()"),nullable=False)); op.create_index("ix_execution_traces_project_id","execution_traces",["project_id"])
def downgrade():
